...
namespace olc
{
	...
	void Sprite::SetPixel(uint32_t x, uint32_t y, Pixel p)
	{
		if ( x < width && y < height)
...
#ifdef UNICODE
					wchar_t sTitle[256];
					swprintf(sTitle, 256, L"PGE - %s - FPS: %d", wsAppName.c_str(), nFrameCount);
#else
#ifndef __MINGW32__
					char sTitle[256];
					sprintf_s(sTitle, 256, "PGE - %s - FPS: %d", sAppName.c_str(), nFrameCount);
#else
					char sTitle[256];
					sprintf(sTitle, "PGE - %s - FPS: %d", sAppName.c_str(), nFrameCount);
#endif
#endif
